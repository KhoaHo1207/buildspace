...
export function DashboardHeader() {
  ...
  const [cachedStats, setCachedStats] = useState<UserStats | null>(null);

  // Load cached stats only on client side after mount
  useEffect(() => {
    const saved = localStorage.getItem("userStats");
    if (saved) {
      setCachedStats(JSON.parse(saved) as UserStats);
    }
  }, []);
    ...
    <header className="border-b bg-white px-6 py-3 dark:bg-gray-900">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          {/* XP */}
          <div className="flex items-center gap-1.5 rounded-full bg-purple-100 px-3 py-1.5 dark:bg-purple-900/30">
